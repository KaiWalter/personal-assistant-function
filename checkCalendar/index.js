...
module.exports = function (context, req) {
  const threshold = parseInt(process.env.THRESHOLD) || 360; // 6 hours
  ...
  totals.forEach((e) => {
    if (e.isBlocked) {
      if (e.total < threshold || e.totalBlocks > 1) {
        // delete block when below threshold or when redundant blocks
        if (e.id) {
      ...
      if (e.total > threshold) {
        // create block when above threshold
    ...
    if (start.getDay() > 0 && start.getDay() < 6) {
      ...
      if (entry) {
        if (e.isAllDay && e.subject === markerSubject) {
          ...
          entry.totalBlocks++;
          entry.id = e.id;
        } else if (e.isAllDay && e.showAs !== "free") {
          ...
          e.subject !== "Freitag ab Eins..."
        ) {
          // only count busy days and non private meetings
...
function calculateTotalMinutes(start, end, entry) {
  // fill slots
  var startSlot =
    start.getHours() * 4 + Math.floor(start.getMinutes() / 15) - 1;
  var endSlot = end.getHours() * 4 + Math.floor(end.getMinutes() / 15) - 1;
  if (end.getMinutes() % 15 !== 0) {
    // fill to 15 minute slot
    endSlot++;
  }
